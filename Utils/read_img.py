...
async def save_upload_file(uploaded_file: UploadFile) -> str:
    ...
    # Using a UUID prevents name collisions and improves security
    file_extension = os.path.splitext(uploaded_file.filename)[1]
    unique_filename = f"{uuid.uuid4()}{file_extension}"
    file_path = os.path.join(temp_dir, unique_filename)

    # 3. Write file in binary chunks (Non-blocking I/O)
    # The 'wb' mode is crucial for saving binary data (like images).
    try:
        async with aiofiles.open(file_path, 'wb') as out_file:
            # Read and write chunks to prevent loading huge files into memory
            # 1024 * 1024 bytes (1MB) is a good chunk size
